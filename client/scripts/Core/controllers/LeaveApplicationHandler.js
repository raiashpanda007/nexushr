...
class LeaveApplicationHandler {
  ...
  async applyLeave(leaveType, startDate, endDate, reason, userId) {
    if (startDate > endDate) {
      return {
        ok: false,
        data: "Start date cannot be after end date",
      };
    }
    if (new Date(startDate) < new Date()) {
      return {
        ok: false,
        data: "Start date cannot be in the past",
      };
    }
    const startSync = await syncQueueHandler.AddItemToQueue(
      "leaveApplications",
      "create",
      {
        leaveType,
    ...
    if (!startSync.ok) {
      return {
        ok: false,
        data:
          "Unable to store in sync queue try to get online cause offline sync queue to gayi: " +
          startSync.data,
      };
    }
    const res = await this.repo.AddLeaveApplication(
      leaveType,
      startDate,
      endDate,
      reason,
  ...
  async AccpetLeaveApplication(leaveAppId) {
    if (this.user.data.user.role !== "HR")
      return { ok: false, data: "Unauthorized Access" };
    const sync = await syncQueueHandler.AddItemToQueue(
      "leaveApplications",
      "updateStatus",
      {
        leaveAppId,
    ...
    const res = await this.repo.UpdateLeaveApplicationStatus(
      leaveAppId,
      "Accepted",
    );
    return res;
